...
- Agent ↔ Backend は HMAC 署名 + タイムスタンプで認証。
- スナップショット・ログは保存/送信前に**シークレット伏字化**（`scrubber.py`）。

### 既知の制約・注意点（要改善）
- **Internal API のバインド**: `main.py` のコメントは「127.0.0.1 専用に別マウント」と記すが、実装は
  メインアプリに同梱されており `0.0.0.0:8000` で公開される。保護は HMAC のみ。公開環境に出す前に
  リバースプロキシ等で `/internal` を遮断すること。
